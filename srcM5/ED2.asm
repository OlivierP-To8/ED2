...
; Buffers.
BUFFER		EQU	$2100	; Buffer général de 1536 octets ($2100 à $26FF).
BUFFERC		EQU	$2100	; Buffer de couleurs de 768 octets ($2100-23FF).
BUFFERF		EQU	$2400	; Buffer de forme de 768 octets ($2400-26FF).

; Init & Packs
MAPADDR		EQU $2100	; Map 1024 octets courante ($2100-$24FF).
INITADR 	EQU $2600	; Adresse d'initialisation
SCROFFSET	EQU $014A	; Offset d'affichage de l'écran de jeu.
SCRMINIMAP	EQU $0727	; Adresse écran de la minimap.
PACKADR 	EQU $8700	; Adresse de chargement des packs d'ennemi.
PACKMAP		EQU $99C0	; Adresse des packs de map.

; Listes d'appel des éléments de décors et des ennemis ($2500-$26FF)
LISTE1		EQU $2500	; Liste de rétablissement des couleurs (25 appels possibles)
PLISTE1		EQU $254B	; Pointeur sur LISTE1.
LISTE2		EQU $254D	; Liste des murs, fonds, trous et sols 1 (30 appels possibles)
PLISTE2		EQU $25A7	; Pointeur sur LISTE2.
LISTE3		EQU $25A9	; Liste des murs, trous et sols 2 (21 appels possibles)
PLISTE3		EQU $25E8	; Pointeur sur LISTE3.
LISTE4_18	EQU $25EA	; Appel pour les ennemis ou objets en G18
LISTE4_19	EQU $25ED	; Appel pour les ennemis ou objets en G19
LISTE4_21	EQU $25F0	; Appel pour les ennemis ou objets en G21
LISTE4_23	EQU $25F3	; Appel pour les ennemis ou objets en G23
LISTE4_24	EQU $25F6	; Appel pour les ennemis ou objets en G24
LISTE4_09	EQU $25F9	; Appel pour les ennemis ou objets en G09
LISTE4_10	EQU $25FC	; Appel pour les ennemis ou objets en G10
LISTE4_12	EQU $25FF	; Appel pour les ennemis ou objets en G12
LISTE4_14	EQU $2602	; Appel pour les ennemis ou objets en G14
LISTE4_15	EQU $2605	; Appel pour les ennemis ou objets en G15
LISTE4_06	EQU $2608	; Appel pour les ennemis ou objets en G06
LISTE4RTS	EQU $260B	; Cloture de liste
LISTE0		EQU $260C	; Pré-liste de rétablissement des couleurs (25 appels possibles)
PLISTE0		EQU $2657	; Pointeur sur LISTE0.
LISTEA1_HP	EQU $2659	; Nombre de HP de l'attaque de front.
LISTEA1		EQU $265A	; Appel de l'attaque de front ou RTS.
LISTEA2_HP	EQU $265D	; Nombre de HP de l'attaque par la droite.
LISTEA2		EQU $265E	; Appel de l'attaque par la droite ou RTS.
LISTEA3_HP	EQU $2661	; Nombre de HP de l'attaque par derrière.
LISTEA3		EQU $2662	; Appel de l'attaque par derrière ou RTS.
LISTEA4_HP	EQU $2665	; Nombre de HP de l'attaque par la gauche.
LISTEA4		EQU $2666	; Appel de l'attaque par la gauche ou RTS.
LISTE_EN	EQU $2669	; Liste des 50 ennemis max du niveau courant = adresse map + PV.

; Valeurs d'initialisation du jeu.
PILE_S		EQU $20CC	; Adresse de la pile système (= STKEND du moniteur).
...
PAS_NORD	EQU $1FC0	; Mise à jour des pas pour l'orientation Nord.
PAS_EST		EQU $1FD0	; Mise à jour des pas pour l'orientation Est.
PAS_SUD		EQU $1FE0	; Mise à jour des pas pour l'orientation Sud.
PAS_OUEST	EQU $1FF0	; Mise à jour des pas pour l'orientation Ouest.

; Données stoquées en mémoire vidéo forme.
W31_DATA	EQU $0029	; Données du mur W31
W41_DATA	EQU $0051	; Données du mur W41
W32_DATA	EQU $0079	; Données du mur W32
W42_DATA	EQU $00A1	; Données du mur W42
W49_DATA	EQU $00C9	; Données du mur W49
H1_DATA		EQU $0122	; Données du trou H1
H3_DATA		EQU $15C2	; Données du trou H3
H5_DATA		EQU $1EF1	; Données du trou H5
H11_DATA	EQU $000A	; Données du trou H11
H12_DATA_PIX EQU $1F25	; Données du trou H12PIX
H14_DATA	EQU $1591	; Données du trou H14
H16_DATA	EQU $0489	; Données du trou H16
H17_DATA	EQU $04B1	; Données du trou H17
H19_DATA	EQU $04D9	; Données du trou H19
H21_DATA_PIX EQU $1541	; Données du trou H21PIX
H23_DATA	EQU $15B9	; Données du trou H23
H24_DATA	EQU $1609	; Données du trou H24
H25_DATA	EQU $0529	; Données du trou H25
H26_DATA	EQU $0579	; Données du trou H26
CH5_DATA	EQU $15EB	; Données du trou CH5
CH7_DATA	EQU $00FB	; Données du trou CH7
CH11_DATA	EQU $1659	; Données du trou CH11
CH13_DATA	EQU $1E01	; Données du trou CH13
CH23_DATA	EQU $1621	; Données du trou CH23
...
;******************************************************************************
	...
	ORG	$2600		; Initialisation à charger entre $2600 et $26FF

	IFEQ INIT-1
		INCLUDE ../src/INIT1.asm	; A sélectionner pour la compilation du pack 1.
	ENDC
	IFEQ INIT-2
		INCLUDE ../src/INIT2.asm	; A sélectionner pour la compilation du pack 2.
	ENDC
	IFEQ INIT-3
		INCLUDE ../src/INIT3.asm	; A sélectionner pour la compilation du pack 2.
	ENDC

;******************************************************************************
;*             VARIABLES ET DONNEES GLOBALES PROTEGEES EN $2700               *
;*                                                                            *
;* Ces variables et données sont utilisables par les packs compilés. Elles ne *
;* Elles ne sont pas écrasées par le buffer en $2100.                         *
;******************************************************************************
...
COULBLD		FCB $17		; Couleurs des tâches de sang (varie selon les monstres).
...
; Sélection video forme. A la suite de LBSR VIDEOC_x, préférer INC $A7C0
VIDEOF:
	LDA	$A7C0
	ORA #%00000001
	STA	$A7C0
...
SET_STATE_INV:
	LDX #$1618		; X pointe l'icône d'invulnérabilité
	BRA SET_STATE_LEV2

SET_STATE_LEV:
	LDX #$161F		; X pointe l'icône de lévitation
SET_STATE_LEV2:
	LBSR VIDEOC_A	; Sélection vidéo couleur.
	LDD #$0028		; A = noir sur fond noir. B = 40 pour les sauts de ligne.
...
DONJON_TEST_TROU1:
	...
	INC	$A7C0		; Sélection vidéo forme + recopie de B lignes de 20 octets.
	BSR DONJON_TEST_TROU_R2	; Recopie de B lignes de 20 octets.
...
; Q = Quitter.
DONJON_Q:
	LBRA ED2Restart	; Redémarrage du jeu
...
DONJON_FEU_BOSS1:
	...
	LDX #$1FE9		; X pointe le buffer de sauvegarde en mémoire vidéo couleur
...
DONJON_SEL1:
	...
	LBSR VIDEOC_A	; Sélection vidéo couleur.
	LDX #$1618		; X pointe l'icône d'invulnérabilité
	LDD #$0628		; A = noir sur fond turquoise. B = 40 pour les sauts de ligne.
	BRA DONJON_SEL2A
...
DONJON_SEL2:
	...
	LBSR VIDEOC_A	; Sélection vidéo couleur.
	LDX #$161F		; X pointe l'icône de lévitation
	LDD #$0328		; A = noir sur fond jaune vif. B = 40 pour les sauts de ligne.
...
L02D:
	...
	INC $A7C0		; Sélection video forme + dessin des formes.	
...
L06UR4_0:
	...
	INC $A7C0		; Sélection video forme puis dessin des formes de L06UR4.
...
L06UR2:
	...
	CLRA			; Couleurs = Encre noire (0) sur fond noir (0)
	BSR L06UR2_1	; Dessin des couleurs de L06UR2.
	LEAX -199,X		; X pointe de nouveau l'échelon central.
	INC $A7C0		; Sélection video forme puis dessin des formes de L06UR2.
...
DP2:
	...
	LBSR DISPLAY_2A_32
	LBSR DISPLAY_2A_9
	LDA #$80		; Couleurs = Encre grise (8) sur fond noir (0)	
...
; Routine de tuile codée
DL1:
	LDA #$30		; Couleurs de serrure = jaune vif (3) sur fond noir (0).
...
; Routine de tuile codée
DL2:
	LDA #$C0		; Couleurs de serrure = bleu (12) sur fond noir (0).
	BRA DL1_1
...
; Routine de tuile codée
DL3:
	LDA #$10		; Couleurs de serrure = rouge (1) sur fond noir (0).
	BRA DL1_1
...
DL4_OFF:
	LDA #$F0		; Couleurs de serrure = orange (15) sur fond noir (0).
	BRA DL4_ON_1
...
DOPEN_R1_1:
	...
	INC $A7C0		; Sélection video forme, puis recopie des formes.
...
TEL06_COL2:			; Haut ou bas de colonne.
	LBSR VIDEOC_A	; Sélection video couleur.
	LDA #$80		; Encre grise sur fond noir.
...
TEL06_DATA1:
	FCB $10,$F0,$B0,$F0,$10,$B0,$F0,$10,$F0,$B0,$F0,$B0,$F0,$10,$F0,$B0,$F0,$10			
...
TEL06_ROT_DATA:
	FCB $6E,$C6,$4C,$04	; Rotation 1	
	FCB $E7,$6E,$C6,$4C	; Rotation 2	
	FCB $70,$E7,$6E,$C6	; Rotation 3	
	FCB $04,$70,$E7,$6E	; Rotation 4	
	FCB $4C,$04,$70,$E7	; Rotation 5	
	FCB $C6,$4C,$04,$70	; Rotation 6
...
CHEST06_DATA:
	FCB $7F			; Couleurs colonnes 1 à 4
	FCB $B1
	FCB $BF
	FCB $3F
	FCB $31
...
CHEST09_DATA:
	FCB $71		; Couleurs pour toutes les colonnes
	FCB $BF
	FCB $31
	FCB $33
	FCB $3F
...
BAN08_1:
	LBSR VIDEOC_A		; Sélection vidéo couleur.
	LDD #$1128			; A = rouge sur fond rouge. B = 40 pour les sauts de ligne.
	STA ,X				; Ligne 1
	STA 1,X
	ABX
...
BAN06:
	...
	LDA #$13			; A = rouge sur fond jaune vif.
...
BAN06_R2:
	BSR BAN06_R1		; Lignes 1 à 46
	LBSR DISPLAY_0A_16	; Lignes 47 à 62
	LBSR DISPLAY_0A_2	; Lignes 63 et 64
	BRA BAN06_R4
	
BAN06_R3:
	BSR BAN06_R1		; Lignes 1 à 46
	LBSR DISPLAY_A0_16	; Lignes 47 à 62
	LBSR DISPLAY_A0_2	; Lignes 63 et 64

BAN06_R4:
	CLRA				; A = noir sur fond noir
	LBSR DISPLAY_2A_11	; Lignes 65 à 75
	LDA #$11			; A = rouge sur fond rouge.
	STA ,X				; Ligne 76
	STA 1,X
	RTS
...
BAN06_DATA:			; Formes des colonnes 5 et 6
	...
	FCB $08,$01		; Couleurs des colonnes 3 et 4
	FCB $81,$11
	FCB $01,$13	

	FCB $81,$11		; Couleurs des colonnes 7 et 8
	FCB $08,$01
	FCB $13,$01	
...
FEN_EFFACE2:
	LDB	$A7C0		; Sélection video forme.
	ORB #%00000001
	STB	$A7C0
...
FEN_MAP:
	...
	LDA #$11		; 1ère ligne rouge sur fond rouge.
	LBSR G2_R1_16
	LDA #$7F		; 128 lignes blanche sur fond orange.	
	LBSR G2_R1_128x16
	LDA #$11		; Dernière ligne rouge sur fond rouge.
	LBSR G2_R1_16
	LDB #40			; Pour les sauts de ligne.
...
DMINIMAP:
	...
	CMPD #$2180		; Adresse < $2180?
	BCC DMINIMAP1	; Non => DMINIMAP1

	ANDB #%00011111	; Oui => adresse fixée de &2160 à $217F.
	ORB  #%01100000

DMINIMAP1:
	CMPD #$2480		; Adresse < $2480?
	BCS DMINIMAP2	; Oui => DMINIMAP2

	ANDB #%00011111	; Non => adresse fixée de &2480 à $249F.
	ORB  #%10000000
...
;------------------------------------------------------------------------------
AFF_INVK:
	LDX #$03B7	; X pointe la clé jaune dans l'inventaire.
	...
	LDX #$03BA	; X pointe la clé bleue dans l'inventaire.
	LDB >INVK2	; B = acquisition de la clé bleue.
	LDA #$0C	; A = noir sur fond bleu.
	BSR AFF_INVK_R1 ; Extinction ou affichage de la clé bleue.

	LDX #$03BD	; X pointe la clé rouge dans l'inventaire.
	LDB >INVK3	; B = acquisition de la clé rouge.
	LDA #$01	; A = noir sur fond rouge + affichage de la clé rouge.
...
AFF_INVK_R1:
	...
	CLRA			; Sinon extinction. A = noir sur fond noir.
...
AFF_INVS:
	...
	LDX #$0FC3		; 4ème ligne du livre.
	LDA ,Y+
	LBSR G2_R1_4x4
	LDX #$121B		; 7ème ligne du livre.
	LDA ,Y+	
	LBSR G2_R1_4x4	
	LDX #$12E3		; 8ème ligne du livre.
	LDA ,Y+	
	LBSR G2_R1_4x4
	LDX #$13AB		; 9ème ligne du livre.
	LDA ,Y+	
	LBSR G2_R1_4x4

	LDX #$0F20		; X pointe l'icone
...
AFF_INVS_DATA:
	FCB $F7,$F7,$F7,$F7	; Lignes 4,7,8 et 9 pleines.
	FCB $71				; Sort blanc sur fond rouge.
	FCB $F0				; Barre graph 25%.
	FDB $0D17			; Localisation des données de forme.

; Boule de feu.
	FCB $77,$77,$F7,$F7	; Lignes 4 et 7 vides. Lignes 8 et 9 pleines.
	FCB $7F				; Sort blanc sur fond orange.
	FCB $F0				; Barre graph 25%.
	FDB $0D19			; Localisation des données de forme.

; Boule de givre.
	FCB $F7,$77,$77,$77	; Ligne 4 pleine. Lignes 7,8 et 9 vides.
	FCB $7C				; Sort blanc sur fond bleu.
	FCB $FC				; Barre graph 37.5%.
	FDB $128F			; Localisation des données de forme.

; Antimatière.
	FCB $77,$F7,$F7,$77	; Lignes 4 et 9 vides. Lignes 7 et 8 pleines.
	FCB $70				; Sort blanc sur fond noir.
	FCB $FF				; Barre graph 50%.
	FDB $1291			; Localisation des données de forme.
...
AFF_INVW_0:
	LBSR VIDEOC_A	; Sélection video couleur.
	LDX #$1BD3		; X pointe l'emplacement des munitions.
	LDD #$FF25		; A = orange sur fond orange. B = 37 pour les sauts.
	...
	LDX #$192B		; X pointe l'emplacement des armes.	
...
AFF_INVW_2:	
	LDX #$1BD5		; X pointe le nombre de munitions à l'écran.
...
AFF_INVW_0B1:
	...
	LDA #$FF		; A = orange sur fond orange
...
AFF_INVW_DATA0B:
	FCB $FF,$F0		; Couleurs colonne 1 et 2
	FCB $FF,$F0
	FCB $FF,$F0
	FCB $FF,$F0
	FCB $FF,$0F
	...
	FCB $0F,$FF		; Couleurs colonne 3 et 4
	FCB $0F,$FF
	FCB $FF,$FF
	FCB $FF,$FF
	FCB $FF,$FF
...
; Arbalette légère
AFF_INVW_DATA1:
	FCB $FF,$FF		; Couleurs colonne 1 et 2
	FCB $FF,$FF
	FCB $FF,$FF
	FCB $FF,$FF
	FCB $FF,$FF
	...
	FCB $F7,$EF		; Couleurs colonne 3 et 4
	FCB $F7,$FE
	FCB $F7,$FE
	FCB $F7,$FE
	FCB $F7,$FE
...
AFF_INVW_DATA3:
	...
	FCB $FF,$F0		; Couleurs colonne 3 et 4
	FCB $FF,$F0
	FCB $00,$00
	FCB $88,$80
	FCB $00,$00
...
AFF_INVO_R1_1:
	LBSR DISPLAY_A0_13 ; Extinction de l'icône.
	LEAX 1,X
...
AFF_INVO1:
	FDB $1791				; Objet 1
	FCB $00,$06,$06,$06,$06,$00
AFF_INVO2:
	FDB $1793				; Objet 2
	FCB $00,$03,$03,$03,$03,$00
AFF_INVO3:
	FDB $1AD8				; Objet 3
	FCB $00,$0F,$07,$0D,$01,$00
AFF_INVO4:
	FDB $1ADA				; Objet 4
	FCB $00,$04,$04,$04,$04,$00
AFF_INVO5:
	FDB $1ADC				; Objet 5
	FCB $00,$0F,$07,$0E,$0C,$00
...
MUS:	
	...
	SWI				; Oui, B = valeur de note
	FCB $1E			; Envoie le code de la note à jouer
...
MUS1:
	...
	STB >$203F		; Variable OCTAVE mise à jour dans le moniteur.
	BRA MUS			; Lit la suite de la partition
...
MUS2:
	...
	ABX
	LDB ,X			; B = durée
	STB >$203C		; Variable DUREE mise à jour dans le moniteur.
	BRA MUS			; Lit la suite de la partition	
...
MUS3:
	...
	STB >$203D		; Variable TIMBRE mise à jour dans le moniteur.
	BRA MUS			; Lit la suite de la partition	
...
MUS4:
	...
	STB >$203A		; Variable TEMPO mise à jour dans le moniteur.
	BRA MUS			; Lit la suite de la partition
...
PRINTO_001_P:
	...
	LDX #$0F20		; X et Y pointent l'icône.
PRINTO_001_P1:	
	LDY #SCROFFSET+$0621	
...
PRINTO_R2:
	LDX #$192B			; X et Y pointe la partie gauche de l'arme.
	LDY #SCROFFSET+$0620
	BSR PRINTO_R1

	LDX #$192D			; X et Y pointe la partie droite de l'arme.
	LDY #SCROFFSET+$0622	
...
PRINTO_R3:
	...
	INC $A7C0		; Sélection vidéo forme et sauvegarde du fond de forme.
PRINTO_R3_1:
...
PRINTO_R4:
	...
	INC $A7C0		; Sélection vidéo forme et restauration du fond de forme.
PRINTO_R4_1:
	LDX #SCROFFSET+$04B7 ; X pointe la zone de fenêtre, colonne 1
	LBSR DISPLAY_2YX_32
	LDX #SCROFFSET+$04B9 ; X pointe la zone de fenêtre, colonne 3
...
CHARS_COUL1	FCB $07,$0B,$03,$03,$0F	; Dégradé de jaune.
CHARS_COUL2	FCB $07,$0E,$06,$06,$0C	; Dégradé de bleu.
CHARS_COUL3	FCB $0E,$0E,$0E,$0E,$0E	; Noir sur fond bleu ciel pour les chaines.
...
ATKPL_W6_1:
	...
	LDA #$22		; A = vert fluo sur fond vert fluo.
...
ATKPL_W3_3:
	...
	LDA >VARDB1
	ORA #$30		; A = jaune sur fond gris ou blanc
	LBSR DISPLAY_A_6
...
ATKPL_W0_3:
	...
	LDD #$11FF		; A = rouge sur fond rouge. B = orange sur fond orange.
	BRA ATKPL_W0_5
...
ATKPL_W0_4:
	LDD #$6677		; A = turquoise sur fond turquoise. B = blanc sur fond blanc
...
ATKPL_W1_R1:
	...
	LDA #$60		; A = turquoise sur fond noir.
	ORA >VARDB1		; A = Turquoise sur fond de mur (trou) ou de sol.
	LBSR DISPLAY_A_3 ; Lignes 1 à 3
	LDA #$30		; A = jaune vif sur fond noir
	ORA >VARDB1		; A = Jaune vif sur fond de mur (trou) ou de sol.
	LBSR DISPLAY_A_4 ; Lignes 4 à 7
	LDA #$30		; A = jaune vif sur fond noir
	ORA >VARDB2		; A = Jaune vif sur fond de sol.	
	STA	,X			; Ligne 8
	ABX
	LDA #$30		; A = jaune vif sur fond noir
	ORA >VARDB1		; A = Jaune vif sur fond de mur (trou) ou de sol.
	LBSR DISPLAY_A_4 ; Lignes 9 à 12
	LDA #$37		; A = jaune vif sur fond blanc
...
PACK_LOAD
    pshs a			; A = numéro de la banque mémoire

	ldb #2
	bsr SetRomPage

    ; copie d'EXO2 en [$2300-$2452]
    ldu #EXO2+$152
    ldx #$B152      ; [$2300-$2452] stocké en [$B000-$B152]
CopyEXO2
    leax -2,x
    ldd ,x
    pshu d
    ...
    puls b			; B = numéro de la banque mémoire
...
ExecOutro
	jmp $B152		; Exécution de l'outro depuis la ROM

LoadPack
	; changement de page avec les données du pack
	bsr SetRomPage

    ; copie de INIT.DAT
    ldu #$2700
    ldx #$EFE0      ; [$2600-$2700] stocké en [$EEE0-$EFE0]
CopyINIT
    leax -2,x
    ldd ,x
    pshu d
    ...
    ; décompression du PACK.DAT.exo2
    ldu #$EEDF      ; dernier octet du PACK1.DAT.exo2
...
GAME_END:
	BSZ $86E5-GAME_END ; Remplissage de zéros jusqu'à 86E4 inclus.


; ED2Restart doit être < à $8700 car il met à jour ce qui est >= $8700
ED2Restart			; Appui du button Init. Porg
	lda #0
	jsr PACK_LOAD
	jmp INITADR
...
PACK_START:
	BSZ $8710-PACK_START ; Remplissage de zéros jusqu'à 870F inclus.
...
;******************************************************************************
	...
	ORG $8710		; Pack à charger entre $8710 et $99BF

	IFEQ PACKE-1
		INCLUDE ../src/PACKE1.asm	; A sélectionner pour la compilation du pack 1.
	ENDC
	IFEQ PACKE-2
		INCLUDE ../src/PACKE2.asm	; A sélectionner pour la compilation du pack 2.
	ENDC
	IFEQ PACKE-3
		INCLUDE ../src/PACKE3.asm	; A sélectionner pour la compilation du pack 3.
	ENDC
...
;******************************************************************************
	...
	ORG $99C0		; Pack à charger entre $99C0 et $9EFF
...
; du fichier EXCEL PACKMAPS.xlsm
	...
	ENDC
	IFEQ PACKM-3
		INCLUDEBIN ../src/PACKM3.bin	; A sélectionner pour la compilation du pack 3.	
	ENDC
