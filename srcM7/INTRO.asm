...
EXO2    EQU $6300
; Adresses page 1
IMG_DEBUT1_FORME_FIN    EQU $0022
IMG_DEBUT2_FORME_FIN    EQU $0024
IMG_MENU_FORME_FIN      EQU $0026
IMG_TITRE_FORME_FIN     EQU $0028
IMG_TITRE_FOND_FIN      EQU $002A
IMG_FORME_FIN           EQU $002C
; Adresses page 2
SAMPLE_EXO2_FIN         EQU $0022
...
BUFW        equ 10
BUF_ECRAN   equ $C000  ; Adresse de la couleur de l'écran de titre
...
AFFICHE_ECRAN
    LDU >IMG_DEBUT1_FORME_FIN       ; U pointe le dernier octet de l'image de forme compressée
    ...
    LDU >IMG_DEBUT2_FORME_FIN       ; U pointe le dernier octet de l'image de forme compressée
    LDY #BUF_ECRAN+$1F40			; Y pointe la fin du buffer
    LBSR EXO2				        ; Décompression de l'image de forme
...
;********************************************************************************
    ...
    LDU >IMG_TITRE_FORME_FIN        ; U pointe le dernier octet de l'image de forme compressée
    LDY #BUF_ECRAN+$1F40			; Y pointe la fin du buffer
    LBSR EXO2				        ; Décompression de l'image de forme
    ...
    LDU >IMG_TITRE_FOND_FIN         ; U pointe le dernier octet de l'image de forme compressée
    LDY #BUF_ECRAN+$1F40            ; Y pointe la fin du buffer
    LBSR EXO2				        ; Décompression de l'image de forme
...
;********************************************************************************
    ...
    LDU >IMG_MENU_FORME_FIN         ; U pointe le dernier octet de l'image de forme compressée
    LDY #BUF_ECRAN+$1F40            ; Y pointe la fin du buffer
    LBSR EXO2				        ; Décompression de l'image de forme
...
AFFICHER_INTRO
    LDU >IMG_FORME_FIN              ; U pointe le dernier octet de l'image de forme compressée
    LDY #BUF_ECRAN+$1F40            ; Y pointe la fin du buffer
    LBSR EXO2                       ; Décompression de l'image de forme

    JSR AFF_MONO                    ; Affichage du buffer

    LDD #$C7C7          ; noir sur fond blanc
    LDY #$4000
SET_FOND
    STD ,Y++
    CMPY #$5F40
...
INTRO_ANIM_FRAME
    ...
    STA $E7CD			    ; envoi l'octet vers la sortie son (253 cycles depuis précédent)
    STU SAMPLE_PTR          ; sauve U sample
    ...
    STA $E7CD			    ; envoi l'octet vers la sortie son (254 cycles depuis précédent)
...
AFF_FEU_LIGNE
    ...
    LDX #$5F40-(BUFFER_FIN-BUFFER)*(40/BUFW)
TEXTE_FEU
    PULU D
    ...
    LDA ,Y+                 ; blanc
    ANDA #$78               ; garde la forme / efface le fond
    ORA #$87                ; remet le fond blanc
    BRA TEXTE_FEU_NEXTA
TEXTE_FEU_ROUGEA
    LDA ,Y+
    ANDA #$78               ; garde la forme / efface le fond
    ORA #$81                ; remet le fond rouge
    BRA TEXTE_FEU_NEXTA
TEXTE_FEU_VIDEA
    LDA ,Y+                 ; si A == 0
...
TEXTE_FEU_NEXTA
    ...
    LDB ,Y+                 ; si blanc
    ANDB #$78               ; garde la forme / efface le fond
    ORB #$87                ; remet le fond blanc
    BRA TEXTE_FEU_NEXTB
TEXTE_FEU_ROUGEB
    LDB ,Y+
    ANDB #$78               ; garde la forme / efface le fond
    ORB #$81                ; remet le fond rouge
    BRA TEXTE_FEU_NEXTB
TEXTE_FEU_VIDEB
    LDB ,Y+                 ; si A == 0
...
MSG_CREDITS
    ...
    FCB     $1B,$4F                  ; double taille
    FCC     "Christophe"
    FCB     $0D,$0A                  ; ligne suivante
    FCB     $1B,$41                  ; couleur forme rouge
    FCB     $1B,$4C                  ; taille normale
...
MSG_TOUCHES_CHOIX_0
    FCB     $1B,$5C                  ; inversion de la vidéo
    FCC     "Retour"
...
MSG_TOUCHES_CHOIX_2
    FCB     $1B,$4C                  ; taille normale (modifiable en $7B)
    FCC     "R"
    FCB     $16,$42                  ; accent aigu
    FCC     "einitialiser"
MSG_TOUCHES_CHOIX_3
