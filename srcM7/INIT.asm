...
; déplacée en $60CC (= STKEND du moniteur).
; =============================================================================
...
;******************************************************************************
DBUFFER		EQU $C000	; Adresse du double buffer pour l'affichage d'images
BUFFER		EQU $BE00	; Adresse du buffer en RAM (pour exécution depuis ROM)
...
;******************************************************************************
; Point d'entrée pour le lancement de l'interface et du jeu (adresse $A156).
INIT1:
	; Suppression du bip des touches.
	LDA #1          ; 0=ON 1=OFF
	STA >$6073

	ORCC #$50		; Désactivation des interruptions.
	CLR >$6036		; Poids fort de la variable OCTAVE du moniteur = 0.

	CLR >$6031		; nettoyage du registre moniteur TEMPO, nécessaire sur TO9.

	LBSR AFFICHE	; Affichage de l'interface + sélection vidéo couleur.

	LDY #TABDECOMP_DATA	; Y pointe la table de décompression.
	LDX #$5F40		; X pointe la partie cachée de la mémoire vidéo couleur.
	LDB #32			; Recopie de la table de décompression.
	BSR INIT_R1

	LDY #GAME_DATA	; Y pointe les données d'initialisation du jeu et du joueur.
	LDX #$5FD4		; X pointe après les touches du clavier.
	LDB #11			; Recopie de 44 octets.
	BSR INIT_R1

	INC $E7C3		; Sélection video forme.
	LDY #TABCOMP_DATA ; Y pointe la table de compression.
	LDX #$5F40		; X pointe la partie cachée de la mémoire vidéo forme.
	LDB #48			; Recopie de la table et des routines d'orientation.
	BSR INIT_R1

	; Tempo de 3 secondes supprimée car il faut 7 secondes pour décompresser ED2
	RTS
...
AFFICHE:
	...
	JSR EXO2		; Décompression de l'image de forme dans le double buffer

	LDU #DBUFFER	; U pointe le début du double buffer
	LDX #$4000		; X pointe le début de la mémoire vidéo forme
	LDB #96
	...
	LDX #$5040		; X pointe la ligne 105 de la mémoire vidéo forme
	LDB #96
	...
	LDX #BUFFER 	; X pointe un buffer de
	LDB #8			; sauvegarde.
	BSR AFFICHE_R2	; Sauvegarde des lignes de forme 97 à 104

	DEC	$E7C3		; Sélection video couleur

	LDU #INTERFACE_F-1 ; U pointe le dernier octet de l'image couleur compressée
	LDY #DBUFFER+$1F40 ; Y pointe la fin du double buffer
	JSR EXO2		; Décompression de l'image couleur dans le double buffer

	LDA #$C0		; Encre noire sur fond noir
	LDB #8			; 8 lignes à noircir
	LDX #$4F00		; X pointe la ligne 97 de la mémoire vidéo couleur
	LBSR AFFICHE_R1	; Noircissement des lignes 97 à 104 à l'écran

	INC	$E7C3		; Sélection vidéo forme

	LDU #BUFFER 	; Y pointe le buffer de sauvegarde
	LDX #$4F00		; X pointe la ligne 97 de la mémoire vidéo forme
	LDB #8
...
AFFICHE_R3:
	...
	INC	$E7C3			; Sélection vidéo forme + affichage de la tuile de forme
AFFICHE_R3_8:
	LDX #$4F0D			; X pointe le texte d'intro à l'écran.
...
MAPCOUL21	FCB $C4		; Couleurs sol/mur de map secondaires, niveau 1.
MAPCOUL22	FCB $C4		; Couleurs sol/mur de map secondaires, niveau 2.
MAPCOUL23	FCB $C4		; Couleurs sol/mur de map secondaires, niveau 3.
MAPCOUL24	FCB $C4		; Couleurs sol/mur de map secondaires, niveau 4.
...
MESSAGE_C: ; Octets de vidéo couleur du message "Patientez SVP"
	FCB $f8,$c0,$c0,$c7,$c0,$c0,$c0,$c0,$c0,$c0,$c7,$f8,$f8 ; 97
	FCB $c7,$c0,$c7,$c0,$c0,$f8,$c7,$c0,$c0,$c0,$c7,$c7,$c7 ; 98
	FCB $43,$43,$43,$43,$43,$43,$43,$43,$43,$c0,$43,$43,$43 ; 99
	FCB $43,$43,$43,$43,$43,$43,$43,$43,$43,$c0,$43,$43,$43 ; 100
	FCB $c3,$c3,$c3,$c3,$c3,$c3,$c3,$c3,$c3,$c0,$c3,$c3,$c3 ; 101
	FCB $c3,$c3,$c3,$c3,$c3,$c3,$c3,$c3,$c3,$c0,$c3,$c3,$c3 ; 102
	FCB $47,$47,$47,$47,$47,$47,$47,$47,$47,$c0,$47,$47,$47 ; 103
	FCB $b8,$c0,$47,$c0,$c0,$c0,$47,$c0,$c0,$c0,$47,$47,$b8 ; 104
...
; création à partir de la version MO :
; tools/exoraw -b -d res/interface.fond.exo2 -o res/interface.fond
; tools/colMO2TO res/interface.fond res/interface.fondTO
; tools/exoraw -b res/interface.fondTO -o res/interface.fondTO.exo2
    INCLUDEBIN ../res/interface.fondTO.exo2

INTERFACE_F: ; Partie vidéo forme de l'interface compressée
    INCLUDEBIN ../res/interface.formeTO.exo2
